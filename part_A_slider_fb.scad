...
// 滑槽参数
slot_depth = 3;       // 槽深度 (mm，从右向左)
slot_height = 3;      // 槽高度 (mm)
slot_length = 200;    // 槽长度 (mm，从前向后)
slot_offset_front = 5; // 槽距离前面 (mm)
slot_offset_top = 10;  // 槽距离上边缘 (mm)
...
difference() {
    ...
    color("Red") {
        // 主滑槽
        translate([width - slot_depth, slot_offset_front, -0.1])
        cube([slot_depth + 0.2, slot_length, slot_height + 0.2]);

        // 后面定位孔（防止滑出）
        translate([width - slot_depth - 0.1, slot_offset_front + slot_length - pin_depth, thickness/2])
        rotate([0, -90, 0])
        cylinder(h = slot_depth + 0.2, r = pin_radius, $fn = 16);

        // 前面定位孔（防止滑出）
        translate([width - slot_depth - 0.1, slot_offset_front + pin_depth, thickness/2])
        rotate([0, -90, 0])
        cylinder(h = slot_depth + 0.2, r = pin_radius, $fn = 16);
    }
}
