...
// 滑槽参数（与A部分匹配）
slot_depth = 3.3;     // 滑舌长度 (mm，比槽深0.3mm)
slot_height = 2.8;    // 滑舌高度 (mm，等于厚度）
slot_length = 200;    // 滑舌长度 (mm)
slot_offset_front = 5; // 滑舌距离前面 (mm)
slot_offset_top = 10;  // 滑舌距离上边缘 (mm)
tolerance = 0.3;        // 配合公差
...
difference() {
    union() {
        ...
        // 滑舌（在左侧，从前向后）
        color("Blue") {
            translate([0, slot_offset_front, 0])
            cube([slot_depth, slot_length, slot_height]);

            // 后面定位销（防止滑出）
            translate([0, slot_offset_front + slot_length - pin_depth, 0])
            cylinder(h = pin_height, r = pin_radius + tolerance, $fn = 16);

            // 前面定位销（对应A部分的孔）
            translate([0, slot_offset_front + pin_depth, 0])
            cylinder(h = pin_height, r = pin_radius + tolerance, $fn = 16);
        }
    }
}
